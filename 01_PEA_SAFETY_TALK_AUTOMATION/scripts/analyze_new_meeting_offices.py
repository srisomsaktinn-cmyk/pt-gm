...
# Grouping by Electricity Authority Offices / Branches
office_groups = defaultdict(list)

for p in participants:
    dept = p['dept']
    if 'กฟส.บปง.' in dept: key = 'กฟส.บ้านโป่ง (การไฟฟ้าส่วนภูมิภาคสาขาบ้านโป่ง)'
    elif 'กฟส.จบง.' in dept: key = 'กฟส.จอมบึง (การไฟฟ้าส่วนภูมิภาคสาขาจอมบึง)'
    elif 'กฟส.ปทอ.' in dept: key = 'กฟส.ปากท่อ (การไฟฟ้าส่วนภูมิภาคสาขาปากท่อ)'
    elif 'กฟส.พธร.' in dept: key = 'กฟส.โพธาราม (การไฟฟ้าส่วนภูมิภาคสาขาโพธาราม)'
    elif 'กฟส.สนผ.' in dept: key = 'กฟส.สวนผึ้ง (การไฟฟ้าส่วนภูมิภาคสาขาสวนผึ้ง)'
    elif 'กฟส.นปม.' in dept: key = 'กฟส.บางแพ / ดำเนินสะดวก (สาขาบางแพ)'
    elif 'กฟส.บ้านคา' in dept: key = 'กฟส.บ้านคา (การไฟฟ้าส่วนภูมิภาคสาขาบ้านคา)'
    elif 'กฟส.ดตก.' in dept: key = 'กฟส.ดำเนินสะดวก (สาขาดำเนินสะดวก)'
    elif 'กฟส.กรญ.' in dept: key = 'กฟส.กาญจนบุรี/กรพะโดน'
    elif 'กฟจ.รบ.' in dept: key = 'กฟจ.ราชบุรี สำนักงานใหญ่จังหวัด (อ.เมืองราชบุรี)'
    elif 'กฟจ.พบ.' in dept: key = 'กฟจ.เพชรบุรี (การไฟฟ้าส่วนภูมิภาคจังหวัดเพชรบุรี)'
    else: key = 'หน่วยงานระดับเขต / ฝ่ายบริหาร / กองต่าง ๆ (กฟต.1)'
    office_groups[key].append(p)

with open(r'd:\Kaeha\new_meeting_office_summary.txt', 'w', encoding='utf-8') as out:
    out.write("===============================================================\n")
    out.write(f" สรุปสาขาการไฟฟ้าของผู้เข้าร่วมประชุมรอบใหม่ (รวม {len(participants)} คน)\n")
    out.write("===============================================================\n\n")

    for key, members in sorted(office_groups.items(), key=lambda x: len(x[1]), reverse=True):
...
print(f"Generated new_meeting_office_summary.txt successfully!")
