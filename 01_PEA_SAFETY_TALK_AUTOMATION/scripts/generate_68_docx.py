...
DOCX_OUT = r'd:\Kaeha\แบบฟอร์มกิจกรรม PEA safety talk [Cybersecurity]_พร้อมรายชื่อ_68คน.docx'

def get_68_participants():
    raw_participants = []
    ...
    for p in raw_participants:
        dept = p['dept']
        # Filter OUT only staff assigned to substations outside Phetchaburi Province
        is_outside_substation = any(kw in dept for kw in outside_substation_codes)
        if not is_outside_substation:
            filtered.append(p)

    # Re-assign sequence 1 to 68
...
def process():
    participants = get_68_participants()
    print(f"Total 68-person participants: {len(participants)}")
    ...
    print(f"Successfully generated 68-person docx: {DOCX_OUT}")
