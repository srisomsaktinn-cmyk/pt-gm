"""
Strategy V2 with ATR-based Initial Hard Stop Loss.
Preserves all Strategy V2 rules 100% while adding a volatility-calibrated safety net.
"""
...
class RSIStrategyV2ATREngine:
    """
    Strategy V2 + ATR-based Hard Stop Loss.
    """
    ...
    def run_backtest(self, candles: List[Candle]) -> "RSIStrategyV2ATREngine":
        self.reset()

        for bar_idx, candle in enumerate(candles):
            # ── 1. BAR OPEN: Execute pending signals from prior close ──
            ...
            if action_trade is not None:
                ...
                # Set initial ATR Stop Loss on entry
            ...
            # ── 2. INTRABAR: Check if active trade hit Hard ATR Stop Loss ──
            active_trade = self.execution.active_trade
            if active_trade is not None and self._active_stop_price is not None:
                stop_hit = False
                fill_price = self._active_stop_price

                if active_trade.direction == PositionSide.LONG:
                    if candle.low <= self._active_stop_price:
                        stop_hit = True
                        # Handle gap down below stop
                        ...
                        active_trade.close(
                            ...
                            exit_reason=f"ATR Stop Loss hit ({self.atr_multiplier}x ATR: ${self._active_stop_price:.2f})"
                        )
                    ...
                    if candle.high >= self._active_stop_price:
                        stop_hit = True
                        # Handle gap up above stop
            ...
            # ── 3. BAR CLOSE: Calculate indicators & state transitions ──
            ...
            # Evaluate state machine transitions & signals
            ...
            # If signal emitted and trade is still open, submit order for next bar open
            if signal is not None:
                # If it's an exit signal and position was already stopped out intrabar, ignore exit signal
